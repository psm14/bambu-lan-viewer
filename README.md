...
1. Tailscale Serve (`docker-compose.tailscale.yml`)
2. Generic reverse proxy / direct edge (`docker-compose.generic.yml`)

Tailscale keeps host ports closed by default. The generic stack binds loopback ports for an external reverse proxy. For full setup instructions:
