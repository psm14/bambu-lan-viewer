...
impl RtspClient {
    ...
    pub async fn start(self) -> anyhow::Result<RtspSession> {
        ...
        if describe.status_code != 200 {
            anyhow::bail!("RTSP DESCRIBE failed: {}", describe.status_code);
        }
        ...
        if setup.status_code != 200 {
            anyhow::bail!("RTSP SETUP failed: {}", setup.status_code);
        }
        let (rtp_channel, rtcp_channel) = parse_interleaved_channels(&setup).unwrap_or((0, 1));
        ...
        if play.status_code != 200 {
            anyhow::bail!("RTSP PLAY failed: {}", play.status_code);
        }
